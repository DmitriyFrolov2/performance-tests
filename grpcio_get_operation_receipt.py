...
# 3. Создаём операцию пополнения счета, привязанную к карте
make_to_up_operation_request = MakeTopUpOperationRequest(
    status=OperationStatus.OPERATION_STATUS_COMPLETED,  # Статус операции
...
make_to_up_operation_response: MakeTopUpOperationResponse = operations_gateway_service.MakeTopUpOperation(
    make_to_up_operation_request
)
